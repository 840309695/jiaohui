...
class ShakeAction extends UserAction
{
    ...
    public function add()
    {
        if (IS_POST) {
            ...
            $this->success('添加成功', U('Shake/index', array(
                'token' => session('token')
            )));
        } else {
            $info               = array();
    ...
    public function edit()
    {
        if (IS_POST) {
            ...
            if ($rt) {
                $this->keyword_model->where(array(
                    'module' => 'Shake',
                    'pid' => $updateWhere['id']
                ))->save(array(
                    'keyword' => $row['keyword']
            ...
            $this->success('修改成功', U('Shake/index', array(
                'token' => session('token')
            )));
        } else {
    ...
    public function del()
    {
        ...
        if ($rt) {
            $this->keyword_model->where(array(
                'module' => 'Shake',
                'pid' => $this->token_where['id']
            ))->delete();
            M('Wall_member')->where(array(
                'act_id' => $this->token_where['id'],
                'act_type' => '2',
                'token' => $this->token
            ))->delete();
            M('shake_rt')->where(array(
                'token' => $this->token,
                'shakeid' => $this->token_where['id']
            ))->delete();
    ...
    public function show_rank()
    {
        ...
        $wecha_id = M('Wall_member')->where(array(
            'token' => $this->token,
            'id' => $this->_get('id', 'intval')
        ))->getField('wecha_id');
        $where    = array(
            'token' => $this->token,
            'shakeid' => $this->_get('pid', 'intval'),
            'wecha_id' => $wecha_id
        );
        if (empty($sceneid)) {
            $where['is_scene'] = '0';
    ...
    public function del_fens()
    {
        ...
        if (M('Wall_member')->where($where)->delete()) {
            ...
            $this->success('删除成功', U('Shake/show_fens', array(
                'token' => $this->token,
                'id' => $pid
            )));
        }
