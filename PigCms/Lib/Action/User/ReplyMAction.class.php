<?php
class ReplyMAction extends UserAction{
	public $token;
...
// 		
	...
	public function reply(){
		...
		$id=$this->_get('msgid');
		$leave_model =M("article_leave");
		$message = $leave_model->where(array('id'=>$id))->getField('info');
		$this->assign("message_id",$id);
		$this->assign('message',$message);
		$where = array("leave_id"=>$id);
		import('ORG.Util.Page');// 导入分页类
	...
	public function deled(){//单项删除
		...
		$res = $leave_model->delete($id);
		if($res){
			$this->success("删除成功",U('User/ReplyM/index',array('wecha_id'=>$this->wecha_id,'token'=>$this->token)));
		}else{
			$this->success("删除失败",U('User/ReplyM/index',array('wecha_id'=>$this->wecha_id,'token'=>$this->token)));
		}
	}
	...
	public function insert(){//添加回复 插入数据库
		...
		if(in_array("0",$result)){
			$this->error("回复失败",U('User/ReplyM/index',array('wecha_id'=>$this->wecha_id,'token'=>$this->token))) ;

		}else{
			$this->success("回复成功",U('User/ReplyM/index',array('wecha_id'=>$this->wecha_id,'token'=>$this->token))) ;
		}
	}
