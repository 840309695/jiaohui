...
class ShakeAction extends WapAction
{
    ...
    {
        if (!in_array($this->act_type, array(
            '2',
            '3'
        )) || !$this->wecha_id) {
            echo '参数错误';
        ...
        if (!$member) {
            header('location:' . U('Scene_member/index', array(
                'token' => $this->token,
                'wecha_id' => $this->wecha_id,
                'id' => $where['act_id'],
                'act_type' => $where['act_type'],
                'name' => 'shake'
            )));
            exit();
        }
        ...
        $thisShake           = $this->shake_model->where(array(
            'token' => $this->token,
            'id' => $id,
            'isopen' => 1
        ))->find();
