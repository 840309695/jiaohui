...
class IndexAction extends WapAction {
	...
	public function lists() {
		...
		if (! empty ( $sub ) and empty ( $imgdata )) {
			...
			$groupid = M ( "Front_user" )->field ( 'group_id' )->where ( array (
					"id" => cookie ( "wapuid" ) 
			) )->find ();
			if (! $this->publics) {
				// 根据权限显示内容
	...
	public function replyadd() {
		...
		$lastime = M ( "Article_reply" )->where ( array (
				"token" => $this->token 
		) )->getField ( "max(time)" );
	...
	public function Leaveadd() {
		...
		$lastime = M ( "Article_leave" )->where ( array (
				"token" => $this->token 
		) )->getField ( "max(time)" );
