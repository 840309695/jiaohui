...
class WallAction extends WapAction
{
    ...
    public function index()
    {
        ...
        if (IS_POST) {
            ...
            if ($thisWall) {
                ...
                $wallRow['time']     = time();
                $wallRowExist        = M('Wall_member')->where(array(
                    'wallid' => intval($thisWall['id']),
                    'wecha_id' => $wallRow['wecha_id']
                ))->find();
                if ($wallRowExist) {
    ...
    public function person()
    {
        ...
        $wallRow['time']     = time();
        $wallRowExist        = M('Wall_member')->where(array(
            'wallid' => $wallRow['wallid'],
            'wecha_id' => $wallRow['wecha_id']
        ))->find();
        if ($wallRowExist) {
            M('Wall_member')->where(array(
                'wallid' => intval($wallRow['wallid']),
                'wecha_id' => $wallRow['wecha_id']
            ))->save($wallRow);
        } else {
            M('Wall_member')->add($wallRow);
