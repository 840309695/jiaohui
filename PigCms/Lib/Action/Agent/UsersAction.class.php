...
class UsersAction extends AgentAction
{
        ...
        if (IS_POST) {
            ...
            if ($users_db->save($_POST)) {
                if ($price) {
                    ...
                    ))->setDec('moneybalance', $price);
                    M('Agent_expenserecords')->add(array(
                        'agentid' => $this->thisAgent['id'],
                        'amount' => (0 - $price),
                        'des' => $users['username'] . '(uid:' . intval($_POST['id']) . ')延期' . $day . '天，共' . count($token) . '个公众号',
                        'status' => 1,
                        'time' => time()
                    ));
                }
    ...
    public function deleteUser()
    {
        ...
        $rt = M('Users')->where(array(
            'id' => $id
        ))->delete();
        if ($rt) {
            $userCount = M('Users')->where(array(
                'agentid' => $this->thisAgent['id']
            ))->count();
            M('Agent')->where($this->agentWhere)->save(array(
                'usercount' => $userCount
            ));
            M('Wxuser')->where(array(
                'uid' => $id
            ))->delete();
            $wxuserCount = M('Wxuser')->where(array(
                'agentid' => $this->thisAgent['id']
            ))->count();
            M('Agent')->where($this->agentWhere)->save(array(
    ...
    public function groupSet()
    {
        ...
        $where             = array_merge($this->agentWhere, array(
            'status' => 1
        ));
        ...
        if (IS_POST) {
            ...
            if (isset($_GET['id'])) {
                $thisGroup = $user_group_db->where(array(
                    'agentid' => $this->thisAgent['id'],
                    'id' => intval($_GET['id'])
                ))->find();
