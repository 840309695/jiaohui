...
class PayAction extends AgentAction
{
    ...
    public function recharge()
    {
        ...
        if (isset($_GET['discountpriceid'])) {
            $thisPrice   = M('Agent_price')->where(array(
                'id' => intval($_GET['discountpriceid'])
            ))->find();
            $buyDiscount = 1;
        ...
        $return_url   = C('site_url') . U('Agent/Pay/return_url', array(
            'discountpriceid' => intval($_GET['discountpriceid'])
        ));
        ...
        $exter_invoke_ip   = "";
        $body              = $subject;
        $data              = M('Agent_expenserecords')->data(array(
        ...
        if ($verify_result) {
            ...
            if ($this->_get('trade_status') == 'TRADE_FINISHED' || $this->_get('trade_status') == 'TRADE_SUCCESS') {
                ...
                if ($indent != false) {
                    ...
                    M('Agent')->where(array(
                        'id' => $indent['agentid']
                    ))->setInc('money', intval($indent['amount']));
                    M('Agent')->where(array(
                        'id' => $indent['agentid']
                    ))->setInc('moneybalance', intval($indent['amount']));
                    $back = M('Agent_expenserecords')->where(array(
                        'id' => $indent['id']
                    ))->setField('status', 1);
                    if ($back != false) {
